...
function setExternalTriggerRunSimulationUnfinishedEvent(event){
	ExternalTriggerRunSimulationUnfinishedEvent = event
}
...
class RunResults {
	...
	static triggerRunFinished() {
		...
		if(currentTime == endTime){
			if(ExternalTriggerRunSimulationFinishedEvent != undefined){
				ExternalTriggerRunSimulationFinishedEvent()
			}
		}else{
			if(ExternalTriggerRunSimulationUnfinishedEvent != undefined){
				ExternalTriggerRunSimulationUnfinishedEvent()
			}
		}
...
class DefinitionEditor extends jqDialog {
	constructor() {
					...
					<div class="table-cell" style="width: 30rem; height: 20rem;">
						<div class="primitive-settings" style="padding: 10px 20px 20px 0px">
							...
							<div class="disable-edit-div">
								<br/>
								<label>
								<input class="disable-edit-checkbox enter-apply" type="checkbox"/>
								Disable Edit</label>
								<div class="disable-edit-note-div"></div>
							</div>
							<div class="disable-delete-div">
								<br/>
								<label>
								<input class="disable-delete-checkbox enter-apply" type="checkbox"/>
								Disable Delete</label>
								<div class="disable-delete-note-div"></div>
							</div>
						</div>
					</div>
		...
		this.disableEditCheckbox = $(this.dialogContent).find(".disable-edit-checkbox").get(0);
		this.disableEditDiv = $(this.dialogContent).find(".disable-edit-div").get(0);
		this.disableEditNote = $(this.dialogContent).find(".disable-edit-note-div").get(0);
		this.disableDeleteCheckbox = $(this.dialogContent).find(".disable-delete-checkbox").get(0);
		this.disableDeleteDiv = $(this.dialogContent).find(".disable-delete-div").get(0);
		this.disableDeleteNote = $(this.dialogContent).find(".disable-delete-note-div").get(0);
	...
	open(id,defaultFocusSelector = null) {
		...
		//Handle get Disable Edit
		let disableEdit = getDisableEdit(this.primitive);
		$(this.disableEditCheckbox).prop("checked",disableEdit);
		this.updateDisableEdit();
	...
	makeApply() {
		if (this.primitive) {
			...
			//handle Disable Edit
			let disableEdit = $(this.disableEditCheckbox).prop("checked");
			setDisableEdit(this.primitive,disableEdit)			
